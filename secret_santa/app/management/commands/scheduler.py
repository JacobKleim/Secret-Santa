...
from app.models import DrawingResult, Game
...
def send_message(sender, recipient, success):
    ...
    logger.info(
        'sending message from {} to {}'.format(sender.tg_id, recipient.tg_id))
    bot_token = os.environ['TELEGRAM_BOT_TOKEN']
    url = f'https://api.telegram.org/bot{bot_token}/sendMessage'
    drawing_result = DrawingResult.objects.filter(
        game__players=recipient).first()

    if success and drawing_result:
        send_date = drawing_result.game.send_date.astimezone(
            pytz.timezone('Europe/Moscow')).strftime('%d-%m-%Y %H:%M')

        text = f'''Розыгрыш проведен!\n
            Вы дарите подарок 🎁 пользователю '
        ...
        text = 'Розыгрыш не был проведен, '
        'слишком мало участников! Подарите подарок себе 🎁'
    data = {
...
def perform_lottery():
    games_to_draw = Game.objects.filter(
        draw_date__lte=timezone.now(), draw_status=False)
    for game in games_to_draw:
        players = list(game.players.all())
        if game.draw_status is True:
            continue
        else:
            if len(players) < 2:
