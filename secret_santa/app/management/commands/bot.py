...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def start(update, context):
            admin_players = Player.objects.filter(
                is_admin=True).values_list('tg_id')

            WHITELIST = [int(tg_id) for tg_id, in admin_players]
            game_id = update.message.text.split(
            ...
            if game_id:
                ...
                if update.message.from_user['id'] in WHITELIST:
                    ...
                    update.message.reply_text(
                        'Вы не организатор! '
                        'Перейдите по ссылке и зарегистрируйтесь как игрок!')
        ...
        def choose_budget(update, context):
            ...
            update.message.reply_text(
                ...
                'регистрации (в формате ММ/ДД/ГГГГ HH:MM):')
        ...
        def create_game(update, context):
            ...
            try:
                ...
                print('Sent POST request to '
                      f'Django for creating game: {game_info}')
                ...
                update.message.reply_text(
                    f'Игра "{data_from_db["name"]}" '
                    'создана! Ссылка на игру: '
                    'https://t.me/assistant_peoples_bot?start={data_from_db["id"]}')
        ...
        def register_user(update, context, game_id):
            context.user_data['game_id'] = game_id
            update.message.reply_text(
                "Давайте начнем регистрацию. Пожалуйста, введите ваше имя:")
        ...
        def create_user(update, context):
            ...
            print(user_info)
            try:
                django_view_url = 'http://127.0.0.1:8000/create_user/'
                response = requests.post(django_view_url, json={
                ...
                print("Sent POST request to "
                      f"Django for creating user: {user_info}")
                print(f"Response from Django: {response.text}")
                ...
                update.message.reply_text(
                    'Вы успешно зарегистрировались в игре!')
                return ConversationHandler.END
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                'GET_BUDGET': [MessageHandler(
                    Filters.text & ~Filters.command, get_budget)],
                'CHOOSE_BUDGET': [MessageHandler(
                    Filters.text & ~Filters.command, choose_budget)],
                'REGISTRATION_DATE': [MessageHandler(
                    Filters.text & ~Filters.command, get_registration_date)],
                'CREATE_GAME': [MessageHandler(
                    Filters.text & ~Filters.command, create_game)],
                'ERROR': [MessageHandler(
                    Filters.text & ~Filters.command, print_error_message)],
                'GET_NAME': [MessageHandler(
                    Filters.text & ~Filters.command, get_name)],
                'GET_LAST_NAME': [MessageHandler(
                    Filters.text & ~Filters.command, get_last_name)],
                'GET_PHONE_NUMBER': [MessageHandler(
                    Filters.text & ~Filters.command, get_phone_number)],
                'CREATE_USER': [MessageHandler(
